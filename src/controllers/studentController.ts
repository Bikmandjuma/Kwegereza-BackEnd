...
import { genderScopeWhere, isOutOfGenderScope } from "../utils/genderScope.js";
...
export const listStudents = asyncHandler(async (req: Request, res: Response) => {
  ...
  const quranLevel = String(req.query.quranLevel ?? "").trim();

  const scopedWhere: any = { role: "STUDENT", ...genderScopeWhere(req.user!) };
  if (status) scopedWhere.status = status;

  const where: any = { ...scopedWhere };
  if (quranLevel) where.quranLevel = quranLevel;
  if (search) {
    where.OR = [
  ...
  // Total male/female counts (ignoring the caller's own gender scope, and
  // ignoring search text these are simple running totals, not a
  // filtered result) so a gender-scoped supervisor can be shown "and N of
  // the other gender" as a count without ever seeing their names/details,
  // and an admin-tier caller (who sees everyone) gets a real breakdown
  // for free instead of having to compute it client-side.
  const genderCountWhere: any = { role: "STUDENT" };
  if (status) genderCountWhere.status = status;

  const [total, students, maleCount, femaleCount, levelGroups] = await Promise.all([
    ...
    prisma.user.count({ where: { ...genderCountWhere, gender: "MALE" } }),
    prisma.user.count({ where: { ...genderCountWhere, gender: "FEMALE" } }),
    // Per-level counts DO respect the caller's own gender scope (unlike
    // maleCount/femaleCount above) a gender-scoped leader clicking
    // "Icyiciro cya Mbere" should see and count only their own gender's
    // students in that level, while admin/super-admin naturally see the
    // combined count for both, since scopedWhere carries no gender
    // restriction for them at all (see genderScopeWhere).
    prisma.user.groupBy({ by: ["quranLevel"], where: scopedWhere, _count: { _all: true } }),
  ]);

  const levelCounts: Record<string, number> = {};
  for (const g of levelGroups) {
    if (g.quranLevel) levelCounts[g.quranLevel] = g._count._all;
  }
  ...
  sendResponse(
    ...
    null,
    { total, page, perPage, totalPages: Math.ceil(total / perPage) || 1, maleCount, femaleCount, levelCounts }
  );
});
...
export function getRangeStart(range: string): Date | null {
